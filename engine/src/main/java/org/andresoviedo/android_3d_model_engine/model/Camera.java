...
public class Camera {
    ...
    public synchronized void animate() {
        ...
        if (method.equals("translate")) {
            ...
            float xPos = (Float) lastAction[1];
            float yPos = (Float) lastAction[2];
            float zPos = (Float) lastAction[3];
            float xView = (Float) lastAction[4];
            float yView = (Float) lastAction[5];
            float zView = (Float) lastAction[6];
            float xUp = (Float) lastAction[7];
            float yUp = (Float) lastAction[8];
            float zUp = (Float) lastAction[9];

//            ArrayList<Float> list = new ArrayList<>();
//            list.add(Math.abs((xPos)));
//            list.add(Math.abs((yPos)));
//            list.add(Math.abs((zPos)));
//
//            list.add(Math.abs( (xView)));
//            list.add(Math.abs((yView)));
//            list.add( Math.abs((zView)));
//            list.add(Math.abs((xUp)));
//            list.add(Math.abs((yUp)));
//            list.add(Math.abs((zUp)));
//            if(animationCounter == 100){
//                animationCounter = Collections.max(list).longValue();
//            }
//            Log.d("max",Collections.max(list).toString());

            float x1 = (xPos-getxPos())/animationCounter;
            float x2 = (yPos-getyPos())/animationCounter;
            float x3 = (zPos-getzPos())/animationCounter;

            float x4 = (xView-getxView())/animationCounter;
            float x5 = (yView-getyView())/animationCounter;
            float x6 = (zView-getzView())/animationCounter;

            float x7 = (xUp-getxUp())/animationCounter;
            float x8 = (yUp-getyUp())/animationCounter;
            float x9 = (zUp-getzUp())/animationCounter;
...
//            zLookDirection /= dp;
    ...
    public synchronized void setCameraPosition(final ArrayList<Float> listPos, boolean animate){

        float xPos = listPos.get(0);
        float yPos = listPos.get(1);
        float zPos = listPos.get(2);
        float xView = listPos.get(3);
        float yView = listPos.get(4);
        float zView = listPos.get(5);
        float xUp = listPos.get(6);
        float yUp = listPos.get(7);
        float zUp = listPos.get(8);
        if(animate){
            lastAction = new Object[]{"setPosition", xPos,  yPos,  zPos,  xView,  yView,  zView,  xUp,  yUp, zUp};
        }
        else{
            setCameraPositionImpl( xPos,  yPos,  zPos,  xView,  yView,  zView,  xUp,  yUp, zUp);
        }

    }
    private void setCameraPositionImpl(float xPos, float yPos, float zPos, float xView, float yView, float zView, float xUp, float yUp,
                                       float zUp){
